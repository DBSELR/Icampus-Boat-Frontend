...
import { Save, Trash2, Edit3, AlertTriangle, HelpCircle, AlertCircle, Section, SquarePen } from "lucide-react";
...
const fetchYear = async (programme: string) => {
  ...
  const onHandleSave = async () => {
    try {
      ...
      const response = await axios.post(
        `${API_BASE}SectionMaster/SaveSectionMaster`, sectionSave
      )
...
const handleEdit = async (sec: any) => {
  ...
  return (
    <div className="dbs-section-container">
      {/* Header */}
      <div className="dbs-section-form-header">
      ...
      <div className="dbs-form-card">
        ...
        <div className="dbs-form-grid-5">
          <div className="dbs-input-box">
            ...
            <select value={selectedProgramme} name="Programme" onChange={(e) => setSelectedProgramme(e.target.value)}>
          ...
          <div className="dbs-input-box">
            <label>Branch</label>
            <select value={selectedBranch} name="BranchCode" onChange={(e) => setSelectedBranch(e.target.value)}>
              <option value="">{loadingBranches ? "Loading..." : "Select Branch"}</option>
              {branchLoad.map((brnh) => (
                <option key={brnh.BRANCHCODE} value={brnh.BRANCHCODE}>
                  {brnh.BRANCHNAME}</option>
              ))}
            </select>
          </div>
          <div className="dbs-input-box">
            <label>Year</label>
            <select value={selectedYear} name="StdYear" onChange={(e) => setSelectedYear(e.target.value)}>
              <option value="">{loadingYears ? "Loading..." : "Select Year"}</option>
              {yearLoad.map((YR) => (
                <option key={YR.ID} value={YR.ID}>{YR.DATA}</option>
              ))}
            </select>
          </div>
          <div className="dbs-input-box">
            <label>Semester</label>
            <select value={sectionInput.Semester} name="Semester" onChange={handleChange}>
          ...
          <div className="dbs-input-box">
            <label>Section</label>
            <input type="text" value={sectionInput.Section} name="Section" onChange={handleChange} />
          </div>
          <div className="dbs-input-box">
            <label>Maximum Management Seats</label>
            <input type="text" value={sectionInput.Mangqutseats} name="Mangqutseats" onChange={handleChange} onKeyUp={addSeats} />
          </div>
          <div className="dbs-input-box">
            <label>Counselling Seats</label>
            <input type="text" value={sectionInput.CouncilingSeats} name="CouncilingSeats" onChange={handleChange} onKeyUp={addSeats} />
          </div>
          <div className="dbs-input-box">
            <label>Lateral Entry</label>
            <input type="text" value={sectionInput.WithLE} name="WithLE" onChange={handleChange} onKeyUp={addSeats} />
          </div>
          <div className="dbs-input-box">
            <label>Fill Seats</label>
            <input type="text" value={sectionInput.fillseats} name="fillseats" onChange={handleChange} />
          </div>
          <div className="dbs-input-box">
            <label>Total Strength</label>
            <input type="text" value={sectionInput.TotStrgth} name="TotStrgth" onChange={handleChange} />
          </div>
        </div>
